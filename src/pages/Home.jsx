...
export const Home = () => {
  ...
  return (
    <>
      <Tabs style={{ marginBottom: 15 }} value={active?0:1} aria-label="basic tabs example">
        <Tab label="Новые" onClick={()=>setActive(true)} />
        <Tab label="Популярные" onClick={()=>setActive(false)} /> 
      </Tabs>
      <Grid container spacing={4}>
        {active ?(<Grid xs={8} item>
          ...
          {(isPostsLoading ? [...Array(5)] : [...Array.isArray(posts.items) ? posts.items : []].sort((a,b)=>b.viewsCount-a.viewsCount)).map((value, index) => isPostsLoading ? <Post key={index} isLoading={true} /> : (
            <Post
              id={value._id}
              title={value.title}
              imageUrl={value.imageUrl ? value.imageUrl: ""}
